...
test('framework-first starter template wires the config-driven app shape', () => {
  ...
  expect(packageJson.scripts.app).toBe('npm run app:dev')
  ...
  expect(packageJson.devDependencies).not.toHaveProperty('tw-animate-css')
  ...
  expect(app).toContain('bridge.system.cpuCount()')
  expect(app).toContain('bridge.native.getStatus()')
  expect(app).toContain('bridge.math.add(2, 3)')
  expect(app).toContain('bridge.math.average(2, 3)')
  expect(app).toContain('bridge.file.hasTxtExtension("notes.txt")')
  expect(app).toContain('bridge.health.isReady()')
  expect(app).toContain('bridge.app.getGreeting()')
  expect(app).toContain('bridge.app.getSummary()')
  expect(tsconfigApp).toContain('.frontron/types/**/*.d.ts')
  expect(fs.readFileSync(join(genPath, 'eslint.config.js'), 'utf-8')).toContain('.frontron/**')
  expect(viteConfig).not.toContain('}),,')
  expect(fs.existsSync(join(genPath, 'components.json'))).toBe(false)
  expect(fs.existsSync(join(genPath, 'src/electron'))).toBe(false)
  expect(fs.existsSync(join(genPath, 'src/components/ui'))).toBe(false)
